...
Route::get('/', function () {
    return view('welcome'); // or redirect('/dashboard')
});
...
Route::prefix('admin')->name('admin.')->group(function () {
    ...
    Route::middleware('auth:admin')->group(function () {
        ...
        Route::put('attendance/{id}', [AdminAttendanceController::class, 'update'])->name('attendance.update')->where('id', '[0-9]+');

        // スタッフ関連
        Route::get('staff/list', [StaffController::class, 'index'])->name('staff.index');
        Route::get('attendance/staff/{id}', [StaffAttendanceController::class, 'index'])->name('staff_attendance.index')->where('id', '[0-9]+');
        Route::get('staff/attendance/{id}/csv', [StaffAttendanceController::class, 'exportCsv'])->name('staff_attendance.csv')->where('id', '[0-9]+');

        // 修正申請関連
        Route::get('stamp_correction_request/list', [AdminRequestController::class, 'index'])->name('stamp_correction_request.index');
